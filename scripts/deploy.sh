...
# prerender phase connects to Payload to render the home page, /portfolio,
# /articles, and the static-text pages at build time (see docker-compose.yml's
# `network: host` on web.build). On normal redeploys postgres is already
# up under restart: unless-stopped, but this guard handles first-deploy
# and the RESET_IMAGE_CACHE path.
