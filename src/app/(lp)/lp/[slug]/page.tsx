...
//
// HTML response cached for 10 min. PR #54 wired DATABASE_URI/PAYLOAD_SECRET
// through to the build container (see Dockerfile + docker-compose.yml's
// web.build config) so prerender no longer fails at build time. Empty
// generateStaticParams keeps the build skipping LP-specific prerender
// (LPs come and go often; rendering them on demand at runtime is the
// right model). Edits in admin call revalidatePath via the LandingPages
// afterChange hook, so changes propagate within ms.
